(() => {
  ...
  const name = () => user?.user_metadata?.full_name || user?.email?.split("@")[0] || "League member";
  ...
  function ensurePlugin() {
    ...
    const plugin=document.createElement("section");plugin.id="channel-plugin";plugin.className="channel-plugin";plugin.innerHTML=channel==="trade-talk"?'<div class="channel-plugin-heading"><div><small>LEAGUE TOOL</small><h2>League trade block</h2><p>Put a rostered player on notice and see who is interested.</p></div><button type="button" data-open-tool>＋ Add player</button></div><div id="channel-plugin-content"></div>':'<div class="channel-plugin-heading"><div><small>MEME STUDIO</small><h2>Create, preview, and post</h2><p>This channel accepts finished memes only—no text-only posts.</p></div><button type="button" data-open-tool>＋ Create a meme</button></div><div id="channel-plugin-content"><p class="plugin-empty">Posted memes appear in the channel below.</p></div>';welcome.insertAdjacentElement("afterend",plugin);$("[data-open-tool]",plugin).onclick=()=>openModal(channel==="memes"?"meme":"trade");if(channel==="trade-talk")loadTradeBlock();
  }
